...
def set_seed(seed: int | None):
    """Make random state deterministic if set"""
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
...
def export_agent(agent: Agent, file: pathlib.Path, sample_obs):
    """Save agent in onnx for inference. Will change agent to eval mode and move agent to cpu."""
    logger.info(f"Exporting onnx to: {file.resolve(True)}")
    ...
    dummy_input = torch.unsqueeze(torch.tensor(sample_obs), 0)
...
def main():
    ...
    if args.cuda and not torch.cuda.is_available():
        warnings.warn("cuda arg is set but CUDA is not available. Using cpu instead")
    ...
    obs = torch.zeros(
        (args.num_steps, num_envs) + envs.single_observation_space.shape
    ).to(device)
    assert envs.single_action_space.shape is not None
    actions = torch.zeros(
        (args.num_steps, num_envs) + envs.single_action_space.shape
    ).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # internally, terminated is same as truncated, which comes from ai_controller.done
            ...
            # when episode is done, save the accumulated rewards over entire track and reset
            for i, d in enumerate(done):
                if d:
                    episode_returns.append(accum_rewards[i])
                    accum_rewards[i] = 0

        # This part causes stutter
    ...
    if args.onnx_export_path is not None:
        ...
        export_agent(agent, path_onnx, envs.single_observation_space.sample())
